...
class SegmentationPreprocessor:
    ...
    def preprocess_case(self, case_path: Path) -> List[Tuple[torch.Tensor, torch.Tensor]]:
        ...
        Returns:
            ...
            - input_tensor has shape (2, D, H, W) containing:
              ...
              - Channel 1: TotalSegmentor kidney mask (includes cysts)
            - target_tensor has shape (1, D, H, W) containing tumor mask
        """
        # Load raw data
        image_chunks, kidney_mask_chunks, tumor_mask_chunks, cyst_mask_chunks = self.load_case(case_path)
        ...
        return self.extract_patches(image_chunks, totalseg_chunks, tumor_mask_chunks, cyst_mask_chunks)
    ...
    def extract_patches(self, image_chunks: List[np.ndarray], 
                       ...
                       tumor_mask_chunks: List[np.ndarray],
                       cyst_mask_chunks: List[np.ndarray]) -> List[Tuple[torch.Tensor, torch.Tensor]]:
        """Extract patches with global normalization."""
        if not image_chunks or not kidney_mask_chunks or not tumor_mask_chunks or not cyst_mask_chunks:
            return []
        ...
        for img_chunk, kidney_chunk, tumor_chunk, cyst_chunk in zip(normalized_chunks, 
                                                                   kidney_mask_chunks,
                                                                   tumor_mask_chunks,
                                                                   cyst_mask_chunks):
            # Combine kidney and cyst masks
            full_kidney_chunk = np.logical_or(kidney_chunk, cyst_chunk).astype(self.processing_dtype)
            ...
            for d in range(0, img_chunk.shape[0] - self.patch_size[0] + 1, self.stride[0]):
                for h in range(0, h_range, self.stride[1]):
                    for w in range(0, w_range, self.stride[2]):
                        ...
                        kidney_patch = full_kidney_chunk[d:d+self.patch_size[0],
                                                       h:h+self.patch_size[1],
                                                       w:w+self.patch_size[2]]
